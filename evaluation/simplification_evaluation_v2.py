...
from perplexity_v2 import score_ppl
...
def load_data(args):
    ...
    if args.hyp_file.endswith('.jsonl'):
        lines = list(iter_lines(args.hyp_file))
        hyp_sents = [i['system'] for i in lines]
    else: # args.hyp_file.endswith('.txt') or args.hyp_file.endswith('.tsv') or args.hyp_file.endswith('.pred'):
    ...
    # otherwise, if no src file and no ref file are provided, assume src and human refs are in hyp file (jsonl format)
        ...
        refs_sents = [i['reference'] for i in lines]
