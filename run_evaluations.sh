...
super_outputs=/srv/scratch6/kew/ats/supervised/results
...
for level in 1 2 3 4; do
    # for split in "dev" "test"; do
        ...
        res=$(python simplification_evaluation.py --src_file $src_files/newsela_manual_v0_v${level}_${split}.tsv --hyp_file $muss_outputs/newsela_manual_v0_v${level}_${split}*.pred)
...
for level in 1 2 3 4; do
    # for split in "dev" "test"; do
    tgt_dir=$super_outputs/newsela_manual_v0_v${level}_${split}
    ...
    if [[ -d $tgt_dir ]]; then
        ...
        for hyp_file in $hyp_files; do
            ...
            if [[ -f "$hyp_file" ]]; then 
                res=$(python simplification_evaluation.py --src_file $src_files/newsela_manual_v0_v${level}_${split}.tsv --hyp_file $hyp_file)
                # if the resul is not empyt, add to full results
                [[ ! -z "$res" ]] && results+=$"${res:312}\n" || echo "failed to score $hyp_file"
            fi
        done
    fi
    # done
            ...
            for split_level in 1 2 3 4; do # we decode all available split levels with conditioners trained on all available levels
                ...
                if [[ -d $tgt_dir ]]; then
                    ...
                    for hyp_file in $hyp_files; do
                        ...
                        # if the relevant file exists, run eval
                        if [[ -f "$hyp_file" ]]; then 
                            res=$(python simplification_evaluation.py --src_file $src_files/newsela_manual_v0_v${split_level}_${split}.tsv --hyp_file $hyp_file)
                            # if the resul is not empyt, add to full results
                            [[ ! -z "$res" ]] && results+=$"${res:312}\n" || echo "failed to score $hyp_file"
                            # echo -e $results
                        fi
...
echo "***** RESULTS *****"
# echo -e $results
echo -e $results | tee "results_on_${split}.csv"
